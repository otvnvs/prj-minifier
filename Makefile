...
WASM_WEB_SRCS:=$(shell if [ -d $(WASM_WEB_DIR) ]; then find $(WASM_WEB_DIR) -type f ! -name '.*.sw?' ! -name '.*.swo'; fi)
...
# --- PERSISTENT BUILD NUMBER GENERATOR PASSTHROUGH ---
BUILD_NUMBER:=$(shell \
...
# --- RECOGNISING WSL (WINDOWS SUBSYSTEM FOR LINUX) ---
...
ifeq ($(GOAL),windows)
    ...
    EXT:=.so
    OPT_FLAGS:=$(NATIVE_OPT_FLAGS) -fPIC
else
    CC:=$(NATIVE_CC)
    PLATFORM:=x86_64-linux-gnu
    EXT:=
...
# Strict Conditional Code Filtering (Shared libraries don't include main.c)
ifeq ($(PLATFORM),wasm32-emscripten)
    SRCS:=$(filter-out $(SRC_DIR)/main.c $(SRC_DIR)/utils.c, $(ALL_SRCS))
else ifeq ($(GOAL),so)
...
.PHONY:all native windows wasm clean test test_windows serve so dll libs
...
# Dynamic Library Generation Targets
so: $(TARGET)
dll: $(TARGET)
libs: so dll
...
wasm:$(OBJS) $(WASM_WEB_DIST)
	...
	@echo "WebAssembly module package built inside ./$(DIST_DIR)"
...
$(TARGET):$(OBJS)
	mkdir -p $(dir $@)
ifeq ($(findstring .so,$(EXT))$(findstring .dll,$(EXT)),)
	# Link Standard Binary Executable
	$(CC) $(CFLAGS) $^ -o $@
else
	# Link Dynamic Shared Library Module
	$(CC) -shared $(CFLAGS) $^ -o $@
endif
	@echo "Successfully linked target module: $@"
endif
